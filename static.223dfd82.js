...
var PlaygroundWrapper_PlaygroundWrapper=function PlaygroundWrapper(props){if(typeof document!=='undefined'){var _require=__webpack_require__(85),Playground=_require.Playground;return external_react_default.a.createElement(Playground,props);}else{return external_react_default.a.createElement(SSRPlayground_SSRPlayground,props);}};
...
var LIB_VERSION=observable_playground_package["a" /* dependencies */].rxjs;var examples={interval:interval,timer:timer,'from':from,'fromPromise':fromPromise,'of':of,defer:defer,'constructor':creation_constructor,filter:filter,find:find,buffer:buffer,mergeMap:mergeMap,switchMap:switchMap,exhaustMap:exhaustMap,concatMap:concatMap,windowToggle:windowToggle,expand:expand,throwError:throwError,'catch':error_handling_catch,retry:retry,retryWhen:retryWhen,onErrorResumeNext:onErrorResumeNext,share:share,zip:zip,forkJoin:forkJoin,combineLatest:combineLatest,tap:tap,finalize:finalize,timeInterval:timeInterval,delay:delay,delayWhen:delayWhen,dematerialize:dematerialize,repeat:repeat,timeout:timeout,pipe:pipe// , 'pausable-buffer': pausableBuffer
};var library={name:'RxJS',version:LIB_VERSION,description:'Reactive Extensions For JavaScript',examples:examples,groups:[{name:'Creation',items:['of','from','fromPromise','interval','timer','defer','constructor']},{name:'Combination',items:['zip','forkJoin','combineLatest']},{name:'Error handling',items:['throwError','catch','retry','retryWhen','onErrorResumeNext']},{name:'Transformation',items:['filter','find','buffer','mergeMap','switchMap','exhaustMap','concatMap','windowToggle','expand']},{name:'Multicasting',items:['share']},{name:'Utility',items:['tap','finalize','timeInterval','delay','delayWhen','dematerialize','repeat','timeout','pipe']// ,   { name: 'Recipes'
//     , items:
//         [ 'pausable-buffer'
//         ]
//     }
...
//# sourceMappingURL=static.223dfd82.js.map